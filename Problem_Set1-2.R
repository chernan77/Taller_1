...
Tabla_2 <- Tabla_2 %>% filter(dsi == 0) 
Tabla_2 <- Tabla_2 %>% filter(pea == 1) 
...
Tabla_3 <- Tabla_2 %>%
  mutate(RME = p6510s1 + p6545s1+p6580s1+p6630s1a1+p6630s2a1+p6630s3a1+p6630s4a1
         +p6630s6a1)
summary(Tabla_3$RME)
Porc0_RME <- (sum(Tabla_3$RME == 0, na.rm = TRUE) / sum(!is.na(Tabla_3$RME))) * 100
...
summary(Tabla_3$RES)
Porc0_RES <- (sum(Tabla_3$RES == 0, na.rm = TRUE) / sum(!is.na(Tabla_3$RES))) * 100

# Renombrar las variables
Tabla_3 <- Tabla_3 %>% rename(c_mne=p6210)
Tabla_3 <- Tabla_3 %>% rename(Edad=age)
Tabla_3 <- Tabla_3 %>% rename(Sexo=sex)
Tabla_3 <- Tabla_3 %>% rename(n_esc_apr=p6210s1)
Tabla_3 <- Tabla_3 %>% rename(c_ocup=p6240)
Tabla_3 <- Tabla_3 %>% rename(exp=p6426)
Tabla_3 <- Tabla_3 %>% rename(n_hsem=hoursWorkUsual)
Tabla_3 <- Tabla_3 %>% rename(Tamaño_empresa=p6870) # No. de Empleados por empresa
Tabla_3 <- Tabla_3 %>% rename(c_cotiz=p6920)
Tabla_3 <- Tabla_3 %>% rename(Horas_trabajadas=totalHoursWorked)
Tabla_3 <- Tabla_3 %>% rename(Ing_Total=ingtot)
Tabla_3 <- Tabla_3 %>% rename(Educ1=maxEducLevel)
Tabla_3 <- Tabla_3 %>% rename(Estrato=estrato1)
Tabla_3 <- Tabla_3 %>% rename(Ingreso_Mon_1=impa)
Tabla_3 <- Tabla_3 %>% rename(Ingreso_Mon_2=isa)
Tabla_3$Educ <- as.integer(Tabla_3$Educ1)
...
Tabla_4 <- Tabla_3 %>%
  select(Estrato, Sexo, Edad, Educ, n_esc_apr, exp, RES, RME, oficio, relab,
         n_hsem, Tamaño_empresa, c_cotiz,pet, Ingreso_Mon_1, Ingreso_Mon_2,
         Ing_Total, c_mne, college, cotPension,
...
Porc_NA <- mean(is.na(Tabla_4$w_hora))* 100 # determinar que % de NA tiene w_hora

cat("La variable w_hora contiene un % de NA=40.32' es:",Porc_NA, "%\n")

Tabla_4 <- Tabla_4[!is.na(Tabla_4$w_hora),] #Se eliminarán las filas con NA para columna w_hora


# Imputar el valor 72 a los valores > 72
Tabla_4$Horas_trabajadas[Tabla_4$Horas_trabajadas > 72] <- 72

#Imputar la moda para que aquellos individuos con exp > 65
Tabla_4$exp[Tabla_4$exp > 65] <- 24
